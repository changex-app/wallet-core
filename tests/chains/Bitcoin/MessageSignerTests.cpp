...
TEST(BitcoinMessageSigner, SignAndVerify) {
...

    {
        ...
        const auto signature = MessageSigner::signMessage(gPrivateKey, address, msg, TWCoinTypeBitcoin);
        EXPECT_EQ(signature, "ILH5K7JQLaRGaKGXXH5mYM6FIIy9IWyY4JUPI+PHYY4WaupxUbg+zy0bhBCrDuehy9x4WidwjkRR1GSLnWvOXBo=");

        EXPECT_TRUE(MessageSigner::verifyMessage(address, msg, signature, TWCoinTypeBitcoin));
    }
    {
    ...
    {
        const auto msg = "test signature";
        const auto signature = MessageSigner::signMessage(gPrivateKey, addressUncomp, msg, TWCoinTypeBitcoin, false);
        EXPECT_EQ(signature, "HLH5K7JQLaRGaKGXXH5mYM6FIIy9IWyY4JUPI+PHYY4WaupxUbg+zy0bhBCrDuehy9x4WidwjkRR1GSLnWvOXBo=");

        EXPECT_TRUE(MessageSigner::verifyMessage(addressUncomp, msg, signature, TWCoinTypeBitcoin));
    }
}
...
TEST(BitcoinMessageSigner, SignNegative) {
    ...
    const auto msg = "test signature";
    // Use invalid address
    EXPECT_EXCEPTION(MessageSigner::signMessage(gPrivateKey, "__THIS_IS_NOT_A_VALID_ADDRESS__", msg, TWCoinTypeBitcoin), "Address is not valid (legacy) address");
    // Use invalid address, not legacy
    EXPECT_EXCEPTION(MessageSigner::signMessage(gPrivateKey, "bc1qpjult34k9spjfym8hss2jrwjgf0xjf40ze0pp8", msg, TWCoinTypeBitcoin), "Address is not valid (legacy) address");
    // Use valid, but not matching key
    EXPECT_EXCEPTION(MessageSigner::signMessage(gPrivateKey, "1B8Qea79tsxmn4dTiKKRVvsJpHwL2fMQnr", msg, TWCoinTypeBitcoin), "Address does not match key");
}
...
TEST(BitcoinMessageSigner, MessageToHash) {
    EXPECT_EQ(hex(MessageSigner::messageToHash("Hello, world!", TWCoinTypeBitcoin)), "02d6c0643e40b0db549cbbd7eb47dcab71a59d7017199ebde6b272f28fbbf95f");
    EXPECT_EQ(hex(MessageSigner::messageToHash("test signature", TWCoinTypeBitcoin)), "8e81cc5bca9862d8b7f22be1f7cb762b49121cf4e1611c27906a041f9a9eb21f");
}
...
TEST(TWBitcoinMessageSigner, SignAndVerify) {
    ...
    const auto signature = WRAPS(TWBitcoinMessageSignerSignMessage(privateKey.get(), address.get(), TWCoinTypeBitcoin, message.get()));
    ...
    EXPECT_TRUE(TWBitcoinMessageSignerVerifyMessage(address.get(), message.get(), signature.get(), TWCoinTypeBitcoin));
}
